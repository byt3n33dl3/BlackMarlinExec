...
def add_user_bh(user, domain, logger, config):
    ...
    if isinstance(user, str):
        ...
        users_owned = user
    if config.get("blackmarlinexec", "bh_enabled") != "False":
        try:
        ...
        uri = f"bolt://{config.get('blackmarlinexec', 'bh_uri')}:{config.get('blackmarlinexec', 'bh_port')}"

        driver = GraphDatabase.driver(
            uri,
            auth=(
                config.get("blackmarlinexec", "bh_user"),
                config.get("blackmarlinexec", "bh_pass"),
            ),
            encrypted=False,
        )
        try:
                ...
                with session.begin_transaction() as tx:
                    for info in users_owned:
                        ...
                        if result.data()[0]["c"].get("owned") in (False, None):
                            ...
                            logger.highlight(f"Node {user_owned} successfully set as owned in blackmarlinexec")
        except AuthError as e:
            logger.fail(f"Provided Neo4J credentials ({config.get('blackmarlinexec', 'bh_user')}:{config.get('blackmarlinexec', 'bh_pass')}) are not valid.")
            return
