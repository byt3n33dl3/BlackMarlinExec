Describe 'ConvertToNetwork' {
    ...
    It 'Translates the string 0/0 to 0.0.0.0/0 (mask 0.0.0.0)' {
        $network = InModuleScope @module { ConvertToNetwork 0/0 }
    ...
    It 'Translates the string 1.2/27 to 1.2.0.0/27 (mask 255.255.255.224)' {
        $network = InModuleScope @module { ConvertToNetwork 1.2/27 }
    ...
    It 'Translates a string containing "3.4.5 255.255.0.0" to 3.4.5.0/16 (mask 255.255.0.0)' {
        $network = InModuleScope @module { ConvertToNetwork "3.4.5 255.255.0.0" }
    ...
    It 'Translates IPAddress argument 1.2.3.4 and SubnetMask argument 24 to 1.2.3.4/24 (mask 255.255.255.0)' {
        $network = InModuleScope @module { ConvertToNetwork 1.2.3.4 -SubnetMask 24 }
    ...
    It 'Translates IPAddress argument 212.44.56.21 and SubnetMask argument 255.255.128.0 to 212.44.56.21/17' {
        $network = InModuleScope @module { ConvertToNetwork 212.44.56.21 255.255.128.0 }
    ...
    It 'Translates IPAddres argument 1.0.0.0 with no SubnetMask argument to 1.0.0.0/32 (mask 255.255.255.255)' {
        $network = InModuleScope @module { ConvertToNetwork 1.0.0.0 }
    ...
    It 'Converts CIDR formatted subnets from <MaskLength> to <Mask>' -TestCases $maskTable {
        ...
        $errorRecord = $null
        try {
            $network = InModuleScope @module { ConvertToNetwork "10.0.0.0/$MaskLength" }
        } catch {
            $errorRecord = $_
        }
    ...
    It 'Converts dotted-decimal formatted subnets from <Mask> to <MaskLength>' -TestCases $maskTable {
        ...
        $errorRecord = $null
        try {
            $network = InModuleScope @module { ConvertToNetwork 10.0.0.0 $Mask }
        } catch {
            $errorRecord = $_
        }
    ...
    It 'Pads a partial subnet mask' {
        $network = InModuleScope @module { ConvertToNetwork "10.0.0.0" "255.255" }

        $network.SubnetMask | Should -Be '255.255.0.0'
    }

    It 'Raises a terminating error when the IP address is invalid' {
        { InModuleScope @module { ConvertToNetwork InvalidIP/24 } } | Should -Throw -ErrorId 'InvalidIPAddress,ConvertToNetwork'
    }

    It 'Raises a terminating error when the mask length is invalid' {
        { InModuleScope @module { ConvertToNetwork "10.0.0.0/33" } } | Should -Throw -ErrorId 'InvalidMaskLength,ConvertToNetwork'
        { InModuleScope @module { ConvertToNetwork "10.0.0.0/-1" } } | Should -Throw -ErrorId 'InvalidMaskLength,ConvertToNetwork'
    }

    It 'Raises a terminating error when the subnet mask is invalid' {
        { InModuleScope @module { ConvertToNetwork "10.0.0.0" "255.255.255.1" } } | Should -Throw -ErrorId 'InvalidSubnetMask,ConvertToNetwork'
    }
}
