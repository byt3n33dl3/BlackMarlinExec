...
BOOL kull_m_pipe_server(LPCWCHAR pipeName, HANDLE *phPipe)
{
	...
	if(ConvertStringSecurityDescriptorToSecurityDescriptor(L"D:(A;OICI;GRGW;;;AU)(A;OICI;GA;;;BA)", SDDL_REVISION, &sa.lpSecurityDescriptor, NULL))
	{
			*phPipe = CreateNamedPipe(pipeName,  PIPE_ACCESS_DUPLEX, PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE | PIPE_WAIT, 1, 0, 0, NMPWAIT_USE_DEFAULT_WAIT, &sa);
